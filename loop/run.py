...
def claude_build(task, existing_code, issues, config, client):
    build_prompt = load_prompt(config["prompts"]["build"])
...
def run_task(task, task_id, config, anthropic_client, openai_client):
    ...
    code = None
    issues = []
    log = {"task": task, "task_id": task_id, "rounds": []}

    for round_num in range(1, config["max_rounds"] + 1):
        print(f"\n[Round {round_num}] Claude building...")
        code = claude_build(task, code, issues, config, anthropic_client)
        ...
        if review.get("approved"):
            print(f"  ✓ Codex approved the code!")
            break
        else:
